...
- [ ] T103 Confirm a verified Supabase production backup completed **before** applying these migrations (release checklist gate, Constitution Principle II / Workflow §4) — _human release action, cannot be self-validated in this environment. The gate + procedure (trigger `mbj-backup-pre-migration`, wait for `VERIFIED`, then push) is documented in `docs/deployment.md` §"Feature 003 — Ordem de migração e backup". Check this box only after the pre-migration backup run reports `VERIFIED`._
